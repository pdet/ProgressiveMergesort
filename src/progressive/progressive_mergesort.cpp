...
ResultStruct ProgressiveMergesort::execute_range_query(Column &original_column, std::pair<int64_t, int64_t> range_query, ssize_t& remaining_swaps){
    ...
    for (size_t j = 0; j < sort_chunks.size(); j++) {
        ...
        if (unsorted_column_count == 0 && sort_chunks.size() > 1) {
            // start a merge
        ...
        if (merge_column) {
            ssize_t todo_merge = std::min((ssize_t) merge_column->data.size() - merge_index,
                                          (ssize_t) DELTA * original_column.size());
            for (size_t j = 0; j < todo_merge; j++) {
                if (left_column < sort_chunks[left_chunk]->qs_index.size &&
                    (right_column >= sort_chunks[right_chunk]->qs_index.size ||
                     sort_chunks[left_chunk]->qs_index.size < sort_chunks[right_chunk]->qs_index.size)) {
        ...
        if (updates->data.size() > original_column.size() / SORTED_COLUMN_RATIO) {
